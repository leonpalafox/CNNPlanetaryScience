function [images, labels, feature_array, image_structure] = read_terrestrial_data(config, pixel_size, resize_scale)
...
images = image_flat;
positive_images = read_terr_data(pixel_size);
labels = master_label;
index = find(strcmp(labels,'TAR')); %Find the current positive examples
labels(index) = []; %delete the labels assigned to them
images(index, :) = []; %delete the images assignedto them
images = [images; positive_images]; %postappend the terrestrial data
terr_labels = repmat({'TAR'}, [1,size(positive_images, 1)]); %Create new set of labels
labels = [labels, terr_labels]; %create the set of labels;
